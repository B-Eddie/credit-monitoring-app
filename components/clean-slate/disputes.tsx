...
export function CleanSlateDisputes() {
  ...
  const activeDisputes = disputes.filter(
    (d) => d.status === "submitted" || d.status === "investigating",
  );
          ...
          <ul className="space-y-5">
            <li className="flex items-start gap-4">
              ...
              <span className="text-sm text-white/50 leading-relaxed">
                Upload supporting documents like receipts or bank statements
              </span>
            </li>
            <li className="flex items-start gap-4">
              <CheckCircle2 className="w-5 h-5 text-[#00D9A4] flex-shrink-0 mt-0.5" />
              <span className="text-sm text-white/50 leading-relaxed">
                Respond promptly to any requests from credit bureaus
              </span>
            </li>
            <li className="flex items-start gap-4">
              <CheckCircle2 className="w-5 h-5 text-[#00D9A4] flex-shrink-0 mt-0.5" />
...
function DisputeCard({ dispute }: { dispute: Dispute }) {
  ...
  const isActive =
    dispute.status === "submitted" || dispute.status === "investigating";
    ...
    <div className="glass-card rounded-3xl p-7">
      <div className="flex items-start justify-between gap-4 mb-5">
        <div className="flex-1">
          <div className="flex items-center gap-3 mb-3 flex-wrap">
            ...
            <Badge
              className={`${stat.bg} ${stat.color} border-0 text-xs h-6 gap-1.5 px-3 font-semibold`}
            >
              {stat.icon}
              {stat.label}
            </Badge>
      ...
      <div className="flex items-center justify-between pt-2">
        ...
        <Button
          variant="ghost"
          size="sm"
          className="h-10 text-[#00D9A4] hover:text-[#00FFB8] hover:bg-transparent p-0 font-semibold"
        >
          Details
